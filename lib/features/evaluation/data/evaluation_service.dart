...
class EvaluationService {
  ...
  Future<List<EvaluationItem>> getStudentCourse() async {
    try {
      Response response = await _client.dio.post(Config.evaluationHandler,
          ...
          if (decoded is List) {
            for (var batch in decoded) {
              if (batch is Map &&
                ...
                items.add(
                    EvaluationItem.fromJson(batch as Map<String, dynamic>));
              }
            }
          }
          return items;
  ...
  Future<List<EvaluationQuestion>> getQuestions(String courseId) async {
    final paperUrl =
        "${Config.evaluationHandler}?action=getPaper&rondom=${DateTime.now().millisecondsSinceEpoch / 1000}";

    Response paperRes = await _client.dio.post(paperUrl,
        data: {"Id": courseId},
        options: Options(contentType: Headers.formUrlEncodedContentType));
    ...
    if (paperData is List && paperData.isNotEmpty) {
      ...
      if (paperData.containsKey("List") &&
          paperData["List"] is List &&
          (paperData["List"] as List).isNotEmpty) {
  ...
  Future<void> submitEvaluation(String id, String answerString) async {
    await _client.dio.post(Config.evaluationHandler,
        data: {"action": "save", "answer": answerString, "Id": id},
        options: Options(contentType: Headers.formUrlEncodedContentType));
  }
}
